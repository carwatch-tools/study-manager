<script lang="ts">
	...
	function handleSubmit() {
		...
		extractZip(files)
			.then((data) => {
				let result = data
					.map((file: { name: string; data: any[] }) => {
						let date = getDateFromFileName(file.name, file.data);
			...
			.catch((err) => {
				const t: ToastSettings = {
					message:
						...
						err +
						'<br>Please make sure you uploaded the correct file(s) and try again.',
					autohide: false
				};
...
>
	<div class="w-1/2 space-y-6 my-6">
		<FileDropzone name="file" id="file" bind:files on:change={handleUpload} accept=".zip" multiple>
